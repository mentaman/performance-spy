...
function reducerChangedTimes(r: Stat): number {
  return Object.values((r?.args as PerfStatsStuff)?.stats || {}).reduce(
    (total, num) => total + (num.count || 0),
    0
  );
}
...
export class Summaries {
  ...
  constructor() {
    ...
    this.allPerfStats = [
      this.selectorsPerfStat,
      this.dispatchPerfStat,
      this.reducersPerfStat,
      this.combinerPerfStat,
    ];
  }
  ...
  getSummary() {
    ...
    const mostChangedReducers = Object.entries(reducersPerfStat.stats)
      .map(([key, value]) => ({
        ...value,
        key,
        changedTimes: reducerChangedTimes(value),
      }))
